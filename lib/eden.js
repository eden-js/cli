...
// Build classes
const PrettyError = new (require('pretty-error'))(); // eslint-disable-line global-require
...
class Eden {
  ...
  async start(opts) {
    ...
    try {
      // Set process name
      process.title = `${config.get('domain')} - ${this.express ? 'express' : 'compute'} #${this.id}`;
    } catch (e) { /* */ }
    ...
    this.on(`${opts.express ? 'express' : 'compute'}.${opts.id}`, (data) => {
      // Emit data
  ...
  _daemons() {
    ...
    for (const file of Object.keys(daemons)) {
      if (this.compute) {
        if (!daemons[file].compute) continue;
        if (daemons[file].compute !== true && !daemons[file].compute.includes(this.id)) continue;
      }

      if (this.express) {
        if (!daemons[file].express) continue;
        if (daemons[file].express !== true && !daemons[file].express.includes(this.id)) continue;
      }
