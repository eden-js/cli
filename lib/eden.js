...
class eden {
  ...
  async lock (key, ttl) {
    ...
    // add async local lock to callbacks
    ...
    // create lock
    let redisLock = await this._lock.redis.lock (config.get ('domain') + '.' + key, ttl);

    // push callbacks
    callbacks.push (redisLock.unlock.bind (redisLock));
  ...
  async _daemons () {
    ...
    for (var file in daemons) {
      ...
      if (this.compute && daemons[file].compute !== true && daemons[file].compute.indexOf (this.id) === -1) continue;
      if (this.express && daemons[file].express !== true && daemons[file].express.indexOf (this.id) === -1) continue;
