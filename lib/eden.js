...
class Eden {
  ...
  async start(opts) {
    ...
    try {
      // Set process name
      process.title = `${config.get('domain')} - ${this.label} #${this.id}`;
    } catch (e) { /* */ }
    ...
    // Emit ready
    this.emit(`eden.${this.label}.${opts.id}.ready`, true);

    // Add ping/pong logic
    this.on('eden.ping', () => {
      // Pong
      this.emit('eden.pong', `${this.label}.${opts.id}`, true);
    }, true);

    // Add thread specific listener
    this.on(`${this.label}.${opts.id}`, (data) => {
      // Emit data
  ...
  thread(type, thread = null) {
    ...
    return {
      call : (str, ...args) => {
        ...
        this.pubsub.emit(`${type}.${thread !== null ? thread : 'all'}`, {
          type   : `connect.call.${str}`,
          args   : [emission],
          callee : `${this.label}.${this.id}`,
        });
  ...
  _events() {
    ...
    this.pubsub.on(`${this.label}:${this.id}`, (channel, data) => {
      // Emit to this
  ...
  _daemons() {
    ...
    const toLoad = daemons.filter((daemon) => {
      // return thread
      return !daemon.thread || daemon.thread === this.label;
