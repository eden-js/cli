...
class Eden {
  ...
  thread (type, thread) {
    // Check ID
    if (thread === null || typeof thread === 'undefined') {
      // Select random thread
      thread = Math.floor(Math.random() * config.get(type + 'Threads'));
    }
  ...
  async lock (key, ttl) {
    ...
    const unlock = await new Promise((resolve) => {
      ...
      this._lock.redis(key, ttl, (unlock) => {
        // Resolve lock
        resolve(unlock);

        // Timeout unlock
        if (ttl) setTimeout(unlock, ttl);
      });
    });
  ...
  async clear (preface) {
    ...
    const keys = await new Promise((resolve, reject) => {
      // Return locks
      this._lock.client.keys(config.get('domain') + '.' + (preface ? preface + '.*' : '*'), (err, keys) => {
        // Check error
        if (err !== null) {
          // Reject
          return reject(err);
