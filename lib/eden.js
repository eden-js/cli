...
class Eden {
   ...
   */
  endpoint (str, fn, all) {
    // Listen on current thread also
    if (all) {
      // On connect call
      this.on('connect.call.' + str, async ({ id, args }, opts) => {
        // Check opts
        if (!opts || !opts.callee) return;

        // Run function
        this.emit(id, await fn(...args, opts), all);
      });
    }
  ...
  async clear (preface) {
    ...
    const keys = await new Promise((resolve, reject) => {
      // Return locks
      this._lock.client.keys(config.get('domain') + '.lock.' + (preface ? preface + '.*' : '*'), (err, keys) => {
        // Check error
        if (err !== null) {
          // Reject
          return reject(err);
