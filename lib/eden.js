...
class eden {
  ...
  constructor (opts) {
    ...
    this.app    = false;
    this.main   = process.env.main ? true : false;
    this.port   = false;
    this.server = false;
    this.router = false;
    this.pubsub = false;
  ...
  _buildApp () {
    ...
    if (this.main && config.main) return this.logger.log ('debug', 'build app skipped', {
      'class' : 'main'
    });
  ...
  _buildLocals () {
    // check if main should build
    if (this.main && config.main) return this.logger.log ('debug', 'build locals skipped', {
      'class' : 'main'
    });
  ...
  _buildView () {
    // check if main should build
    if (this.main && config.main) return this.logger.log ('debug', 'build view skipped', {
      'class' : 'main'
    });
  ...
  async _buildRouter () {
    // check if main should build
    if (this.main && config.main) return this.logger.log ('debug', 'build router skipped', {
      'class' : 'main'
    });
    ...
    for (var b = 0; b < routes.length; b++) {
      ...
      if (route.type === 'post') {
        // set type
        var upType = route.upload && route.upload.type || 'array';
        ...
        // create upload middleware
        var upload = this.uploader[upType].apply (this.uploader, upApply);
      ...
      args.push (async (req, res, next) => {
        ...
        // run acl middleware
        var aclCheck = await acl.middleware (req, res);
        if (aclCheck === 0) {
          // nexted
          return next ();
        } else if (aclCheck === 2) {
  ...
  async _buildDaemons () {
    // check if main should build
    if (!this.main && config.main) return this.logger.log ('debug', 'build daemons skipped', {
      'class' : 'sub'
    });
  ...
  async _getPort () {
    // check if main should build
    if (this.main && config.main) return this.logger.log ('debug', 'get port skipped', {
      'class' : 'sub'
    });
