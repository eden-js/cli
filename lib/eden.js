  ...
  async controller(file) {
    ...
    try {
      ...
      if (!this.register('controller')[file]) {
        ...
        // do endpoints
        if (callable.length) callable.forEach((endpoint) => {
          // do endpoint
          this.endpoint(endpoint.endpoint, this.register('controller')[file][endpoint.fn], endpoint.all);
        });

        // do endpoints
        if (eventable.length) eventable.forEach((e) => {
          // do endpoint
          this.on(e.event, this.register('controller')[file][e.fn], e.all);
        });

        // do endpoints
        if (hookable.length) hookable.forEach((hook) => {
          // do endpoint
          this[hook.type](hook.hook, this.register('controller')[file][hook.fn]);
        });
      }
