...
class eden {
  ...
  async _buildRouter () {
    // build router
    this.acl = (this.acl ? this.acl : express.Router ());
    ...
    for (var b = 0; b < routes.length; b++) {
      // set route
      let route = routes[b];
      let path  = (route.mount + route.route).replace ('//', '/').replace (/\/$/, '');

      // add to acl
      this.acl[route.type] (path, (req, res, next) => {
        // set route
      ...
      // add route
      this.router[route.type] (path, (req, res, next) => {
        try {
          // try run function
          this._ctrl[route.class][route.fn] (req, res, next);
        } catch (e) {
          // set error
          this.error (e);
  ...
  async _buildDaemons () {
    ...
    for (var i = 0; i < daemons.length; i++) {
      // require daemon
      var daemon = await this.require (daemons[i]);
