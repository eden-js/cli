...
class eden {
  ...
  constructor (opts) {
    ...
    this.pubsub = false;
    this.events = false;
    this.isMain = process.env.main === 'true' ? true : false;
  ...
  _registerEventEmitter () {
    // get connection
    var conn = JSON.parse (JSON.stringify (config.redis || {}));
  ...
  _buildApp () {
    ...
    if (this.isMain && config.main) return this.logger.log ('debug', 'build app skipped', {
      'class' : 'eden'
    });
  ...
  _buildLocals () {
    // check if main should build
    if (this.isMain && config.main) return this.logger.log ('debug', 'build locals skipped', {
      'class' : 'eden'
    });
  ...
  _buildView () {
    // check if main should build
    if (this.isMain && config.main) return this.logger.log ('debug', 'build view skipped', {
      'class' : 'eden'
    });
  ...
  async _buildRouter () {
    // check if main should build
    if (this.isMain && config.main) return this.logger.log ('debug', 'build router skipped', {
      'class' : 'eden'
    });
  ...
  async _getPort () {
    // check if main should build
    if (this.isMain && config.main) return this.logger.log ('debug', 'get port skipped', {
      'class' : 'eden'
    });
