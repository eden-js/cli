...
class eden {
  ...
  callAll (str, ...args) {
    ...
    return new Promise ((resolve) => {
      ...
      this.on (id, (result) => {
        ...
        if (results.length >= this._endpoints[str]) {
          // resolve results
          resolve (results);
        }
      }, all);
   ...
   * @param  {Boolean}  all
   */
  endpointAll (str, fn, all) {
    // set endpoint
    this._endpoints[str] = 0;

    // on endpoint add
    this.on (`connect.call.all.${str}.add`, () => {
      // increment endpoint count
      this._endpoints[str]++;
    }, all);

    // emit endpoint add
    this.emit (`connect.call.all.${str}.add`, all);
