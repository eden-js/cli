...
class eden {
  ...
  constructor () {
    ...
    this.endpointAll = this.endpointAll.bind (this);
  ...
  callAll (str, ...args) {
    ...
    const result = new Promise ((resolve) => {
      ...
      // on message
      this.on (id, async (result) => {
        // push result to results
        results.push (result);

        // lock endpoint
        const lock = await this.lock (`lock.${str}`);

        // set count
        const count = parseInt (await this.get (`endpoint.${str}`, () => {
          // return default
          return 1;
        }));
    ...
    // emit to socket
    this.emit (`connect.call.all.${str}`, emission, all);
  ...
  /**
   * create RPC all endpoint
   *
   * @param  {String}   str
   * @param  {Function} fn
   * @param  {Boolean}  all
   */
  async endpointAll (str, fn, all) {
    // lock endpoint
    const lock = await this.lock (`lock.${str}`);
    ...
    // increment count
    await this.set (`endpoint.${str}`, count + 1);

    // set on endpoint
    this.on (`connect.call.all.${str}`, async (emission) => {
      // run endpoint
      this.emit (emission.id, await fn (...emission.args), all);
    }, all);
