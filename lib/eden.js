...
const EdenModel    = require('@eden-js/model');
...
class Eden {
  ...
  async start(opts) {
    ...
    try {
      // Set process name
      process.title = `${config.get('domain')} - ${this.thread} #${this.id}`;
    } catch (e) { /* */ }
    ...
    // Bind Eden classes
    this.router = this.port ? new Router() : false;

    // Build daemons
    await this._daemons();

    // Emit ready
    this.emit(`eden.${this.thread}.${opts.id}.ready`, true);

    // Add ping/pong logic
    this.on('eden.ping', () => {
      // Pong
      this.emit('eden.pong', `${this.thread}.${opts.id}`, true);
    }, true);

    // Add thread specific listener
    this.on(`${this.thread}.${opts.id}`, (data) => {
      // Emit data
  ...
  register(name, value) {
    ...
    if (typeof value === 'undefined') {
      // Return register
      return dotProp.get(this._register, name);
    }

    // Set register value
    this._register = dotProp.set(this._register, name, value);
  ...
  on(str, fn, all) {
    ...
    if (all) {
      // On connect call
      this.events.on(str, (...args) => {
        // Check last arg
        if (!args[args.length - 1] || !args[args.length - 1].callee) return;
  ...
  endpoint(str, fn, all) {
    // On connect call
    this.on(`connect.call.${str}`, async ({ id, args }, opts) => {
      // Check opts
      if (opts && opts.callee) args.push(opts);

      // Run function
      this.emit(id, await fn(...args), !!((opts && opts.callee) || all));
    }, all);
  }
   ...
   * @return {*}
   */
  thread(type, thread = null) {
    let setThread = thread;

    // Check ID
    if (setThread === null) {
      // Select random thread
      setThread = Math.floor(Math.random() * config.get(`${type}Threads`));
    }
    ...
    return {
      call : (str, ...args) => {
        ...
        this.pubsub.emit(`${type}.${setThread}`, {
          type   : `connect.call.${str}`,
          args   : [emission],
          callee : `${this.thread}.${this.id}`,
        });
  ...
  async lock(key, ttl) {
    ...
    const unlock = await new Promise((resolve) => {
      // Lock with redis lock
      this._lock.redis(key, setTtl, (redisUnlock) => {
        // Resolve lock
        resolve(redisUnlock);

        // Timeout unlock
        if (setTtl) setTimeout(redisUnlock, setTtl);
      });
    });
  ...
  async get(key, notCached, ttl) {
    ...
    // Check key
    if (value === null && setNotCached) {
      // Set cached
      value = await setNotCached();

      // Set
      if (setTtl) {
        await this.set(key, value, setTtl);
      } else {
  ...
  async set(key, value, ttl = 0) {
    ...
    // Check value
    if (value instanceof require('model') && value.get('_id')) { // eslint-disable-line global-require
      // Set value
      setValue = {
  ...
  async clear(preface) {
    ...
    const keys = await new Promise((resolve, reject) => {
      // Return locks
      this._lock.client.keys(`${config.get('domain')}.${preface ? `${preface}.*` : '*'}`, (err, lockKeys) => {
        // Check error
        if (err !== null) {
          // Reject
          return reject(err);
    ...
    for (const key of keys) {
      ...
      await new Promise((resolve, reject) => {
        // Delete key
        this._lock.client.del(key.slice((`${config.get('domain')}.`).length), (err, res) => {
          // Check error
          if (err !== null) {
            // Reject
            reject(err);
  ...
  _caches() {
    ...
    // Create cache
    this.register('cache', new RedisStore(conn));
  }
  ...
  _events() {
    ...
    this.pubsub.on(`${this.thread}:${this.id}`, (channel, data) => {
      // Emit to this
  ...
  _daemons() {
    ...
    // get daemons
    const toLoad = daemons.filter((daemon) => {
      // return thread
      return !daemon.thread || daemon.thread === this.thread;
    }).sort((a, b) => (b.priority || 0) - (a.priority || 0));

    // loop toload
    for (const daemon of toLoad) {
      // Run daemon
      try {
        // Require Daemon
    ...
    for (const file of Object.keys(daemons)) {
      // continue
      if (daemons[file].thread !== this.thread && daemons[file].thread) continue;
