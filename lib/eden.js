...
class eden {
  ...
  async _buildRouter () {
    ...
    // build acl router
    this.acl = (this.router ? this.router : express.Router ());
    ...
    var userCtrl = await this.controller ('/lib/bundles/user/controllers/main');
    ...
    for (var i = 0; i < priorities.length; i ++) {
      ...
      for (var type in types) {
        ...
        for (var route in routeType) {
          ...
          // add to acl
          this.acl[type] (path, (req, res, next) => {
            // set route
          ...
          // run route action
          this.router[type] (path, ctrl[routeType[route].action]);
        }
      }
    }

    // set routes to app
    this.app.use ('/',    this.acl);
    this.app.use ('/ajx', this.acl);

    // do acl function
    this.app.use (acl.middleware);
  ...
  async _buildDaemons () {
    ...
    for (var i = 0; i < daemons.length; i++) {
      // require daemon
      var daemon = await this.require (global.appRoot + daemons[i]);

      // run daemon
      try {
        this._daemon[daemons[i]] = new daemon (this);

        // log daemon
        this.logger.log ('info', 'running daemon ' + this._daemon[daemons[i]].constructor.name);
      } catch (e) {
        // print error
        this.error (e);

        // log daemon
        this.logger.log ('error', 'daemon ' + this._daemon[daemons[i]].constructor.name + ' failed!');
      }
    }
  }
  ...
  async controller (file) {
    ...
    try {
      ...
      if (!this._ctrl[file]) {
        // require controller
        var ctrl = await this.require (global.appRoot + file);
