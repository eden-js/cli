...
class eden {
  ...
  _buildThread () {
    // set if main thread
    if (this.port.toString () === config.port.toString ()) {
      // set this process as main thread
      this.main = true;

      // main thread started
      this.logger.log ('debug', 'started main eden thread');
    } else {
      // set this process as not main thread
      this.main = false;

      // main thread started
      this.logger.log ('debug', 'started sub eden thread');
    }
  }
