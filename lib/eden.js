...
class Eden {
  ...
  endpoint (str, fn, all) {
    // On connect call
    this.on ('connect.call.' + str, async ({
      id, args
    }) => {
      // Run function
   ...
   * Emit to main thread
   *
   * This method emits an event to the main (first) thread. To use this simply:
   *
   * `this.eden.main ([event name], [..arguments])`
   *
   * @param {String} str
   * @param {Object} obj
   *
  ...
  async clear () {
    ...
    for (const key of keys) {
      ...
      await new Promise ((resolve) => {
        // Delete key
        this._lock.client.del (key, (err, res) => {
          return resolve (res);
        });
      });
    }
