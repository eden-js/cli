...
class configUtil {
  ...
  _fn (line, lines) {
    ...
    while (!isFn) {
      ...
      if (lines[line].indexOf ('(') > -1 && lines[line].indexOf (')') > -1) {
        ...
        // check if function has async
        if (isFn.indexOf (' ') > -1) {
          isFn = isFn.split (' ')[isFn.split (' ').length - 1];
        }
      }
    }
  ...
  _acl (tags) {
    ...
    for (var i = 0; i < tags.length; i++) {
      ...
      if (tag.tag == 'acl') {
        // push to test
        acl.test.push ((tag.name === 'false' || tag.name === 'true') ? (tag.name === 'true') : tag.name);
      }

      // test fail
      if (tag.tag == 'fail') {
        // push to fail
        acl.fail = tag.type && tag.type.length ? safeEval ('({' + tag.type + '})') : {
          'redirect' : tag.name
        };
      }
    }
    ...
    if (acl.test.length && (acl.test[0] === true || acl.test[0] === false)) {
      // set for logged in/out
      acl.test = acl.test[0];
    ...
    // return acl
    return acl.test.length ? acl : false;
  }
    ...
    for (var i = 0; i < routes.length; i++) {
      ...
      for (var y = 0; y < mounts.length; y++) {
        ...
        rtn[priority][(routes[i].type ? routes[i].type : 'get')][rt] = {
          'controller' : '/' + (chunk.path.indexOf ('/app') > -1 ? 'app' : 'lib') + '/bundles' + chunk.path.split ('bundles')[1].replace (/\\/g, '/'),
          'action'     : fn
        };
      }
    }
