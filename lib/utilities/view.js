...
class engine {
  ...
  render (filePath, options, callback) {
    // create remove array
    var remove = [
      'page', 'cache', 'route', 'layout', '_locals', 'settings', 'menus', 'head', 'routeStart', 'url', 'path', 'isJSON', 'user'
    ];
    ...
    var render = {
      ...
      'mount' : {
        ...
        'layout' : (options.layout ? options.layout : 'main') + '-layout'
      },
      'timer' : {
        'route' : {
    ...
    // check menus
    if (options.user)  render.user = options.user;
    if (options.menus) render.menus = options.menus;
