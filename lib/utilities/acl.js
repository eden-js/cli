...
class aclUtil {
  ...
  async middleware (req, res, next) {
    // check route has acl
    if (!res.locals.route || !res.locals.route.acl || !res.locals.route.acl.length) return next ();

    // loop acl for tests
    for (var i = 0; i < res.locals.route.acl.length; i ++) {
      // check acl
      var check = await this.validate (req.user, res.locals.route.acl[i]);
      ...
      if (!check) {
        ...
        if (res.locals.route.fail) {
          ...
          return res.redirect (res.locals.route.fail);
        }
