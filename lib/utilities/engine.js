...
class engine {
  ...
  constructor () {
    // require all tags
    let tagFiles = glob.sync (global.appRoot + '/app/cache/views/**/*.{tag,mixin.js}');
  ...
  render (filePath, options, callback) {
    // require riot tag
    options.mountPage = filePath.split ('views/')[1].split (path.sep).join ('-').trim ().replace ('.tag', '') + '-page';
