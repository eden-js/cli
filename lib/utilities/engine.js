...
class engine {
  ...
  render (filePath, options, callback) {
    ...
    // log timing
    global.logger.log ('debug', options.route.route + ' loaded in ' + (options.routeEnd - options.routeStart) + 'ms', {
      'class' : 'route ' + options.route.class + '.' + options.route.fn
    });
    ...
    try {
      ...
      // log rendered
      global.logger.log ('debug', options.route.route + ' rendered in ' + (new Date ().getTime () - renderStart) + 'ms', {
        'class' : 'route ' + options.route.class + '.' + options.route.fn
      });
