...
async function initEden(suppliedDirType = null, migrateGit = false) {
  ...
  if (dirType === 'oldApp' && shouldMakeIntoApp) {
    ...
    await Promise.all([
      ...
      fs.remove(path.join(process.cwd(), 'Dockerfile')),
      fs.remove(path.join(process.cwd(), '.dockerignore')),
      fs.remove(path.join(process.cwd(), 'kubernetes.yml')),
      fs.remove(path.join(process.cwd(), 'docker-compose.yml')),
      fs.remove(path.join(process.cwd(), '.gitlab-ci.yml')),
      fs.remove(path.join(process.cwd(), '.travis.yml')),
