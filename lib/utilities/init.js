...
async function initEden(suppliedDirType = null, migrateGit = false) {
  ...
  // Regardless of type, create a bundles directory
  if (!(await fs.pathExists(path.join(process.cwd(), 'bundles')))) {
  ...
  // Make sure any type has an edenconfig.js (good for identification too)
  if (!(await fs.pathExists(path.join(process.cwd(), 'edenconfig.js')))) {
