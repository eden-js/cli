...
class Parser {
  ...
  controller(chunk) {
    ...
    for (const com of comments) {
      // Check if class
      if (com.comment.code.includes('class')) {
        // Set class
        returns.classes[file].name = com.comment.code.split('class')[1].split('extends')[0].trim();
        returns.classes[file].desc = com.description;

        // Check for mount
        if (com.acls) {
          returns.classes[file].acl = this._acl(com.acls);
        }
      ...
      if (com.routes) {
        // Set function
        let fn = com.comment.code.split('(')[0].trim();
        ...
        for (let b = 0; b < com.routes.length; b += 1) {
          ...
          if (com.caches) {
            ...
            const lowCacheBType = cacheBDesc.type.toLowerCase();
            const lowCacheBDesc = cacheBDesc.toLowerCase();

            routeCache = {
              ttl  : com.ttl ? parseInt(com.ttl[0].description, 10) : (60 * 60 * 1000),
              name : cacheB.type ? lowCacheBDesc.trim() : `${lowRouteBType.trim()}:${routeBDesc.trim()}`,
              type : cacheB.type ? lowCacheBType : cacheBDesc.replace(/\{|\}/g, '').trim(),
            };
          }
          ...
          const route = {
            fn,
            acl      : com.acls ? classF.acl.concat(this._acl(com.acls)) : classF.acl,
  ...
  daemon(chunk) {
    ...
    for (const com of comments) {
      // Check if class
      if (!com.comment.code.includes('class')) continue;

      // Set class
      returns.daemons[file].name = com.comment.code.split('class')[1].split('extends')[0].trim();
  ...
  task(path) {
    ...
    for (const com of comments) {
      ...
      // Set class
      task.name = com.comment.code.split('class')[1].split('extends')[0].split('{')[0].trim();
      task.desc = com.description;

      // Check for mount
      if (com.tasks) task.task = com.tasks[0].description;
      if (com.watchs) task.watch = true;

      // Do afters
      if (com.afters) {
        // Loop afters
        for (const after of com.afters) {
          // Add afters
          task.after.push(after.description);
        }
      }
      ...
      if (com.befores) {
        // Loop afters
        for (const before of com.befores) {
          // Add afters
          task.before.push(before.description);
        }
      }
    }
  ...
  _uploads(tags) {
    ...
    if (updatedTags.length > 1) {
      ...
      upload.tag = (updatedTags[0].type && updatedTags[0].type ? updatedTags[0].type : 'array');
    }
