...
class configParser {
  ...
  parse (chunk) {
    ...
    let file     = (chunk.path.indexOf ('/app') > -1 ? 'app' : 'lib') + '/bundles' + chunk.path.split ('bundles')[1].replace (/\\/g, '/');
    ...
    for (var a = 0; a < comments.length; a++) {
      ...
      if (comments[a].comment.code.indexOf ('class') > -1) {
        ...
        // check for mount
        if (comments[a].acls)       returns.classes[file].acls     = this._acl (comments[a].acls);
      ...
      if (comments[a].routes) {
        ...
        for (var b = 0; b < comments[a].routes.length; b++) {
          ...
          let route = {
            'fn'       : fn,
            'acl'      : comments[a].acls ? returns.classes[file].acls.concat (this._acl (comments[a].acls)) : returns.classes[file].acls,
  ...
  _acl (tags) {
    ...
    for (var i = 0; i < tags.length; i++) {
      // add to returns
      returns.push (tags[i].description);
    }
