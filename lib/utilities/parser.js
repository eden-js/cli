...
class EdenParser {
        ...
        for (let routeI = 0; routeI < routeTags.length; routeI += 1) {
          ...
          if (cacheTags.length > 0) {
            ...
            routeCache = {
              ...
              name : cachePath ? cachePath.toLowerCase().trim() : `${lowRouteType.trim()}:${routeTag.name.trim()}`,
          ...
          const route = {
            ...
            offline  : com.p.offline !== undefined && (com.p.offline.val === 'true' || com.p.offline.parts.length === 0),
          };
  ...
  daemon(chunk) {
    ...
    if (com.p.compute !== undefined && (com.p.compute.val === 'true' || com.p.compute.parts.length === 0)) {
      // Set compute true
      compute = true;
    } else if (com.p.compute !== undefined && com.p.compute.parts.length > 0) {
