...
class parser {
  ...
  daemon (chunk) {
    ...
    for (var a = 0; a < comments.length; a++) {
      ...
      // check for mount
      if (comments[a].express) returns.daemons[file].express = comments[a].express === 'true' || comments[a].express === true ? true : (parseInt (comments[a].express) || false);
      if (comments[a].compute) returns.daemons[file].compute = comments[a].compute === 'true' || comments[a].compute === true ? true : (parseInt (comments[a].compute) || false);
    }
