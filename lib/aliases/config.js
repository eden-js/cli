...
const configLocations = loader.getFiles('edenconfig.js', loader.getLocations(nconf.get('modules') || [], 'bundles').map(p => p.slice(0, -'bundles/'.length)));
  ...
  for (const key of Object.keys(configFile)) {
    ...
    if (existing !== undefined) {
      nconf.set(key, deepMerge(existing, configFile[key], {
        // Reverse merger so edenconfig.js values go first
        arrayMerge : (dest, source) => source.concat(dest),
      }));
    } else {
