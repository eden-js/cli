...
class middleware {
  ...
  middleware (store, next, action) {
    ...
    // check action
    if ([mongorito.ActionTypes.REMOVE, mongorito.ActionTypes.CREATE, mongorito.ActionTypes.UPDATE].indexOf (action.type) === -1) return next (action);

    // hook create
    if (store.model) return new Promise ((resolve) => {
      // run hook
      eden.hook (store.model.constructor.name + '.' + action.type.toLowerCase ().split ('/')[1], store.model, () => {
        // resolve next action
  ...
  __get (store, next, action) {
    ...
    if (hasModel) {
      ...
      return new Promise (async (resolve) => {
        ...
        for (let i = 0; i < field.length; i++) {
          // check field type
          if (field && field[i].id && field[i].model) {
            // return promise
