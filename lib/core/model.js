...
const DbModel = require ('edenjs-model').DbModel;
...
class Model extends DbModel {
  ...
  async mutex (ttl) {
    ...
    this.eden.logger.log ('debug', 'locking ' + this.constructor.name + ' #' + this.get ('_id').toString (), {
      'class' : this.constructor.name
    });

    // Create mutex
    const mutex = await eden.lock (this.constructor.name + '.' + this.get ('_id').toString (), (ttl || 60 * 1000));

    // Refresh model after mutex
    await this.refresh ();

    // Log locked to debug
    this.eden.logger.log ('debug', 'locked ' + this.constructor.name + ' #' + this.get ('_id').toString (), {
      'class' : this.constructor.name
    });
    ...
    this.unlock = () => {
      ...
      this.eden.logger.log ('debug', 'unlocked ' + this.constructor.name + ' #' + this.get ('_id').toString (), {
        'class' : this.constructor.name
      });
  ...
  static async __sanitiseObject (value, ...keys) {
    ...
    if (Array.isArray (value)) {
      ...
      if (!keys || !keys.length) {
        // Return value
        return isCore ? (value.sanitise ? await value.sanitise () : undefined) : value;
      }
  ...
  async __sanitiseModel (...keys) {
    ...
    // Update to hook
    toHook[this.constructor.name] = this;

    // Hook model sanitise
    await this.eden.hook (`${this.constructor.name}.sanitise`, toHook);
