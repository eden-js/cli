...
class middleware {
  ...
  __set (store, next, action) {
    ...
    for (var key in action.fields) {
      ...
      if (Array.isArray (action.fields[key])) {
        // map field
        action.fields[key] = action.fields[key].filter ((field) => field).map (field => this.__sanitise (store.model, field));

        // check array
        if (!action.fields[key]) action.fields[key] = [];
        if (!Array.isArray (action.fields[key])) action.fields[key] = [action.fields[key]];
      } else {
  ...
  async __load (mod, field) {
    ...
    if (!mod.children[field.model]) mod.children[field.model] = {};

    // check model
    if (mod.children[field.model][field.id]) {
      // resolve field
      found = mod.children[field.model][field.id];
      ...
      // set found
      mod.children[field.model][field.id] = found;
    }
  ...
  __sanitise (mod, field) {
    ...
    // set field value
    if (!mod.children) mod.children = {};
    if (!mod.children[field.constructor.name]) mod.children[field.constructor.name] = {};

    // set value
    mod.children[field.constructor.name][field.get ('_id').toString ()] = field;
