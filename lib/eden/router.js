...
class router {
  ...
  async _default (app) {
    ...
    app.use ((req, res, next) => {
      ...
      // add assets
      res.header ('Link', [
        '</public/css/app.min.css' + (config.get ('version') ? '?v=' + config.get ('version') : '') + '>; rel=preload; as=style',
        '</public/js/app.min.js' + (config.get ('version') ? '?v=' + config.get ('version') : '') + '>; rel=preload; as=script'
      ].join (','));
