...
class Router {
  ...
  _view(app) {
    // Set view engine
    app.set('views', `${global.appRoot}/cache/views`);
  ...
  async _router(app) {
    ...
    // Set router classes
    const routerClasses = Object.keys(classes).map(key => classes[key]);
  ...
  __upload(route) {
    ...
    return this._multer[upType](...upApply);
  }
