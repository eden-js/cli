...
class router {
  ...
  async build () {
    ...
    ['use', 'get', 'post', 'push', 'delete', 'all'].forEach ((method) => {
      ...
      this[method] = function () {
        // call express method
        this._express[method] (...arguments);
      };
    });

    // run express build methods
    let methods = ['_default', '_api', '_view', '_router', '_error'];
  ...
  async _default (app) {
    ...
    app.use ((req, res, next) => {
      ...
      // check isJSON request
      req.isJSON = res.isJSON = res.locals.isJSON = (req.headers.accept || req.headers.Accept || '').indexOf ('application/json') > -1;
  ...
  async _router (app) {
    ...
    // set router classes
    let routerClasses = Object.keys (classes).map ((key) => {
      // return key class
      return classes[key];
    });
  ...
  __route (route) {
    ...
    // push cache
    if (route.cache) args.push ((req, res, next) => {
      ...
      if (type === 'all' || (type === 'anonymous' && !req.user)) {
        // check eden cache
        return eden.get (name + route.cache.strung + (req.isJSON ? 'json' : '')).then ((cached) => {
          // check cached
          if (cached) {
            // log cache hit
            eden.logger.log ('debug', path + ' cache hit in ' + (new Date ().getTime () - res.locals.timer.start) + 'ms', {
          ...
          // log cache hit
          eden.logger.log ('debug', path + ' cache miss in ' + (new Date ().getTime () - res.locals.timer.start) + 'ms', {
            'class' : (route && route.type) ? route.type.toUpperCase () + ' ' + route.class + '.' + route.fn : 'no route'
          });
