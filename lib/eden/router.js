...
class router {
  ...
  async _default (app) {
    ...
    app.use ((req, res, next) => {
      ...
      // add assets
      if (req.originalUrl.indexOf ('/ajx/') !== 0 && req.originalUrl.indexOf ('/api/') !== 0) res.header ('Link', [
  ...
  _error (app) {
    ...
    app.use (async (req, res) => {
      ...
      // await hook
      await eden.hook ('404', opts);

      // render 404 page
      res.render ('missing', opts);
    });
  }
  ...
  async __route (route) {
    ...
    args.push (async (req, res, next) => {
      ...
      try {
        ...
        // try run function
        return await controller[route.fn] (req, res, next);
      } catch (e) {
        // set error
        eden.error (e);
        ...
        return res.render ('error', {
          'trace'   : e.stack && await acl.validate (req.user, 'admin') ? e.stack.split ('\n').slice (1) : null,
          'message' : e.toString ()
        });
      }
    });
