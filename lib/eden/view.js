  ...
  async render (path, options, callback) {
    ...
    try {
      ...
      await eden.hook ('view.foot', ({ render, foot }), () => {
        ...
        foot += '<!-- USER.START --><script data-eden="before-script">window.eden.user = JSON.parse (decodeURIComponent (atob ("' + new Buffer (encodeURIComponent (JSON.stringify (render.user))).toString ('base64') + '")));</script><!-- USER.END -->';
