...
class view {
  ...
  async render (path, options, callback) {
    ...
    if (options.isJSON) {
      ...
      // check cache
      if (route.cache && (route.cache.type === 'all' || (route.cache.type === 'anonymous' && !req.user))) {
        // set cache
        await eden.set (route.cache.name + route.cache.strung + 'json', JSON.stringify (render));
      }
    ...
    try {
      ...
      // create foot string
      await eden.hook ('view.foot', foot, () => {
        // render foot
        foot += '<script data-eden="before-script">window.eden = JSON.parse (decodeURIComponent (atob ("' + new Buffer (encodeURIComponent (JSON.stringify (render))).toString ('base64') + '")));</script>';
        foot += '<script type="text/javascript" src="' + (config.get ('cdn.url') || '/') + 'public/js/app.min.js' + (config.get ('version') ? '?v=' + config.get ('version') : '') + '" data-eden="script"></script>';
      ...
      if (route.cache && (route.cache.type === 'all' || (route.cache.type === 'anonymous' && !req.user))) {
        // set cache
        await eden.set (route.cache.name + route.cache.strung, page);
      }
