...
class View {
  ...
  async render (path, options, callback) {
    ...
    try {
      ...
      let userFrontend = JSON.stringify(render.user);
      ...
      let renderFrontend = JSON.stringify(render);
      ...
      await eden.hook('view.foot', ({
        ...
        // Add to foot
        foot += '<!-- DATA.START --><script data-eden="before-user">window.eden = JSON.parse (decodeURIComponent (atob ("' + new Buffer(encodeURIComponent(renderFrontend)).toString('base64') + '")));</script><!-- DATA.END -->';
        foot += '<!-- USER.START -->';

        // Check user
        if (render.user && Object.keys(render.user).length) foot += '<script data-eden="before-script">window.eden.user = JSON.parse (decodeURIComponent (atob ("' + new Buffer(encodeURIComponent(userFrontend)).toString('base64') + '")));</script>';
