...
class View {
  ...
  async render (path, options, callback) {
    ...
    try {
      ...
      let userFrontend = render.user ? JSON.stringify(render.user) : null;
      ...
      await eden.hook('view.foot', ({
        ...
        // Check user
        if (userFrontend) foot += '<script data-eden="before-script">window.eden.user = JSON.parse(decodeURIComponent(atob("' + new Buffer(encodeURIComponent(userFrontend)).toString('base64') + '")));</script>';
