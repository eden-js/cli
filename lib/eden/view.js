  ...
  async render(path, options, callback) {
    ...
    try {
      ...
      await eden.hook('view.foot', ({
        ...
        // Add to foot
        foot += `<!-- DATA.START --><script data-eden="before-user">window.eden = JSON.parse (decodeURIComponent(atob("${Buffer.from(encodeURIComponent(renderFrontend)).toString('base64')}")));</script><!-- DATA.END -->`;
        foot += '<!-- USER.START -->';

        // Check user
        if (userFrontend) foot += `<script data-eden="before-script">window.eden.user = JSON.parse(decodeURIComponent(atob("${Buffer.from(encodeURIComponent(userFrontend)).toString('base64')}")));</script>`;
