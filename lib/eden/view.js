...
class view {
  ...
  async render (path, options, callback) {
    ...
    // create remove array
    var remove = [
      'res', 'page', 'cache', 'route', 'layout', '_locals', 'settings', 'head', 'routeStart', 'url', 'path', 'isJSON', 'timer'
    ];
    ...
    // log timing
    eden.logger.log ('debug', options.path + ' start in ' + (render.timer.route - render.timer.start) + 'ms', {
