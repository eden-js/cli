...
class router {
  ...
  constructor () {
    ...
    jQuery (document).ready (() => {
      ...
      this.history.replace ({
        'state'    : {
          'state' : window.eden.state || false,
          'mount' : window.eden.mount || false
        },
        'pathname' : window.eden.mount.url || ''
      });
  ...
  load (data) {
    ...
    // set title
    document.title = (data.page.title ? data.page.title + ' | ' : '') + window.eden.config.title;
    ...
    for (var key in data) {
      // trigger
      this._store.trigger (key, data[key]);
    }
    ...
    // set window state
    window.eden.state = data.state;
    window.eden.mount = data.mount;
  }
  ...
  submit (form) {
    // get url
    let url = form.attr ('action') || window.location.href.split (window.eden.config.domain)[1];
