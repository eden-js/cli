...
class metaController extends controller {
  ...
  async _middleware (req, res, next) {
    ...
    req.meta = res.meta = (type, opts) => {
      ...
      // get name
      let name = opts.id || opts.itemprop || opts.property || opts.href || opts.name || opts[Object.keys (opts)[0]];
    ...
    // create description
    req.article = res.article = (name, content) => {
      // add description
      req.meta ({
    ...
    req.title = res.title = (title) => {
      ...
      // add description
      req.og ('title', req.t (title));
      req.meta ({
        'content'  : req.t (title),
        'itemprop' : 'name'
      });
      req.twitter ('title', req.t (title));
    ...
    // set default
    req.og ('url', 'https://' + config.get ('domain') + req.url.replace ('/ajx', ''));
