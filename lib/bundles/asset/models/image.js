...
class image extends model {
  ...
  async download (link) {
    ...
    let fil = fs.createWriteStream (global.appRoot + '/app/cache/' + tmp);
  ...
  thumb (label) {
    // check if exists
    if (!fs.existsSync (this._path (true) + '/' + this.get ('hash') + this.get ('ext'))) return false;
  ...
  async _remove (next) {
    ...
    // check if files
    if (fs.existsSync (global.appRoot + '/www/public/' + this._path ()) && !fs.readdirSync (global.appRoot + '/www/public/' + this._path ()).length) {
      // remove directory
      fs.rmdirSync (global.appRoot + '/www/public/' + this._path ());
    }

    // check if files
    if (fs.existsSync (path.dirname (global.appRoot + '/www/public/' + this._path ())) && !fs.readdirSync (path.dirname (global.appRoot + '/www/public/' + this._path ())).length) {
      // remove directory
