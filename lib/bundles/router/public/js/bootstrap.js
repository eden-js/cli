...
class router {
  ...
  async initialize () {
    ...
    // await mount
    await this._store.hook ('frontend.mount', window.eden);
    ...
    this.history.listen (async (location) => {
      ...
      if (location.state && Object.keys (location.state).length > 0) {
        ...
        // do layout
        await this._store.hook ('layout', location.state);
  ...
  async load (data) {
    // await hook
    await this._store.hook ('load', data);
  ...
  async submit (form) {
    ...
    // hook form
    await this._store.hook ('submit', url, opts);
  ...
  async update (state) {
    ...
    if (old.pathname === state.url) {
      ...
      // hook form
      await this._store.hook ('state', old);
