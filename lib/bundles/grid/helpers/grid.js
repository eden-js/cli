  ...
  _bind () {
    ...
    ['where', 'or', 'and'].forEach ((method) => {
      ...
      this[method] = function () {
        // check where
        if (!that._where.query) {
          // set where
          that._where = that._where[method] (...arguments);
        ...
        if (method === 'and') {
          ...
          // set and
          that._where.query = extender (Array.from (arguments)[0], that._where.query);
        } else {
          that._where = that._where[method] (...arguments);
        }
      };
