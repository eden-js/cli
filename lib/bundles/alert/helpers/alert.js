...
class AlertHelper extends Helper {
  ...
  async _emitAlert (socketType, socketReceiver, type, message, opts = {}, medium = 'helper') {
    ...
    if (opts && opts.save) {
      ...
      // Emit to socket
      socketReceiver.emit(data);
    } else {
      // Emit to redis
      socket[socketType](socketReceiver, data);
    }
  }
  ...
  async user (user, type, message, opts = {}, medium = 'helper') {
    // Emit alert to user
    await this._emitAlert('user', user, message, opts, medium);
  }
  ...
  async socket (socket, type, message, opts = {}, medium = 'helper') {
    ...
    await this._emitAlert('socket', socket, message, opts, medium);
  }
  ...
  async session (session, type, message, opts = {}, medium = 'helper') {
    ...
    await this._emitAlert('session', session, message, opts, medium);
  }
