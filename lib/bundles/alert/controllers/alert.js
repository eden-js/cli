...
class alertController extends controller {
  ...
  _socket (opts) {
    ...
    opts.alert = (type, data) => {
      // send user alert
      return socket.session (opts.socket.request.cookie[config.get ('session.key') || 'eden.session.id'], 'alert', {
        'type' : type,
        'opts' : data
      });
    };
  ...
  async _middleware (req, res, next) {
    // create alert function
    req.alert = res.alert = (type, opts) => {
      // send user alert
      return socket.session (req.sessionID, 'alert', {
        'type' : type,
        'opts' : opts
      });
    };
