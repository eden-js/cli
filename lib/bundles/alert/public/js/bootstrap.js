...
class alert {
  ...
  constructor () {
    ...
    socket.on ('alert', async (data) => {
      ...
      // run alert
      if (!data.prevent) this.emit (data.type, data.opts);
    });

    // run alert on socket Event
    socket.on ('notify', async (title, opts) => {
      // hook alert
      await store.hook ('notify', data);

      // run alert
      if (!data.prevent) this.notify (title, opts);
    });
  ...
  async emit (type, opts) {
    ...
    if (document.hidden && this._notify && opts.type !== 'alert') {
      // document is hidden
