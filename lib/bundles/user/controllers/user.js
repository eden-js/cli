...
class userController extends controller {
  ...
  build () {
    ...
    // on user create
    this.eden.pre ('user.create', this._acl);
  ...
  loginSubmitAction (req, res, next) {
    ...
    passport.authenticate ('local', (err, User, info) => {
      ...
      req.login (User, async () => {
        ...
        // send alert
        alert.session (req.sessionID, 'success', 'Successfully logged in', {
          'save' : true
        });
        ...
        console.log (await User.model ('acl'), await User.get ('acl'));
  ...
  async registerSubmitAction (req, res) {
    ...
    // check username
    if (req.body.username && req.body.username.trim ().length < 5) {
      // send alert
  ...
  async _user (req, res, next) {
    ...
    if (req.user) res.locals.user.acl =  await aclUtil.list (req.user);
