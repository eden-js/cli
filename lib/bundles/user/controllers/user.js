...
class userController extends controller {
  ...
  async _user (req, res, next) {
    // set user locally
    if (!req.isJSON || req.authChanged) res.locals.acl  = await acl.list (req.user);
    if (!req.isJSON || req.authChanged) res.locals.user = req.user ? await req.user.sanitise () : false;
