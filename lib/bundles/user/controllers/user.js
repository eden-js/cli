...
class userController extends controller {
  ...
  build () {
    ...
    // pre login
    this.eden.pre ('user.login',  this._login);
    this.eden.pre ('user.logout', this._logout);

    // pre login fail
    this.eden.on ('user.login.fail', (obj) => {
      // run login fail
      this._login (obj.user, obj.message);
    });
  ...
  async logoutAction (req, res) {
    // hook user login
    await this.eden.hook ('user.logout', req.user);
  ...
  async registerFormAction (req, res) {
    ...
    // hook user login
    await this.eden.hook ('user.register', User);
   ...
   *
   * @param {user} User
   * @param {Boolean|String} fail
   */
  async _login (User, fail) {
    // add to login
    let Login = new login ({
      'way'  : 'login',
      'user' : User,
