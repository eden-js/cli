...
class user extends model {
  ...
  async _acl () {
    ...
    // check default acl
    let Default = await aclModel.where ({
    ...
    if (!Default) {
      // add new acl
      Default = new aclModel (config.get ('acl.default'));
    ...
    if (!count) {
      // check first acl
      var Admin = await aclModel.where ({
      ...
      if (!Admin) {
        // add new acl
        Admin = new aclModel (config.get ('acl.first'));
