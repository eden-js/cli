...
class acl {
  ...
  validate (tests, list) {
    ...
    return !tests.find ((test) => {
      // check if true/false
      if (test === true && !store.get ('user').exists ()) return true;
      if (test === false && store.get ('user').exists ()) return true;
  ...
  list () {
    ...
    if (!store.get ('user').exists ()) return [];

    // get groups
    let acls = [];
    let Acls = store.get ('user').get ('acls') || [];
