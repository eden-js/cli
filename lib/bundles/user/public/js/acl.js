...
class acl {
  ...
  validate (tests, list) {
    ...
    return !tests.find ((test) => {
      // check if true/false
      if (test === true && (!store.get ('user') || !store.get ('user').id)) return true;
      if (test === false && (store.get ('user') && store.get ('user').id)) return true;

      // check list
      if (list === true) return false;

      // loop props
      return !dotProp.get (obj, test);
    });
  }
  ...
  list () {
    ...
    if (!store.get ('user') || !store.get ('user').id) return [];
